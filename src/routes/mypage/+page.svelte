...
<main class="mx-auto flex w-full max-w-5xl flex-col gap-10 px-5 pb-16 pt-14 sm:px-8">
  <header class="glass-panel space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-4">
      ...
      <div
        class="flex items-center gap-3 rounded-3xl border border-slate-200/70 bg-white/90 px-4 py-3 shadow-sm"
      >
        <img
  ...
  <section class="grid gap-4 lg:grid-cols-3">
    ...
    >
      ...
      <p class="text-sm text-slate-600">
        내가 작성한 프로필과 동료들이 남긴 추천서를 한 화면에서 확인하고, 필요한 내용을 바로
        수정하세요.
      </p>
      {#if profile?.updated_at}
        <p class="text-xs text-peer-indigo">
    ...
    {#if invitesEnabled}
      ...
      >
        ...
        <p class="text-sm text-slate-600">신뢰하는 동료들을 초대해보세요.</p>
      </a>
    {:else}
      <div
        class="flex h-full flex-col justify-between gap-3 rounded-3xl border border-dashed border-slate-300/70 bg-slate-50/70 px-6 py-6 text-slate-500"
      >
        <div class="space-y-1">
          <span class="text-xs font-semibold uppercase tracking-wide text-slate-400">초대</span>
        ...
        <p class="text-sm">
          베타 초대 기능을 점검 중입니다. 곧 동료를 초대할 수 있도록 안내드릴게요.
        </p>
      </div>
    {/if}
