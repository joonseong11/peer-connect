...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  if (profileError) {
    ...
    preferences = {
      endorsements: notificationRow.notify_endorsements ?? DEFAULT_PREFERENCES.endorsements,
...
const parseBooleanField = (values: FormDataEntryValue[], fallback: boolean): boolean => {
...
const parsePreferences = (formData: FormData): NotificationPreferences => ({
  endorsements: parseBooleanField(
    formData.getAll(COLUMN_MAP.endorsements),
    DEFAULT_PREFERENCES.endorsements
  ),
  gatherings: parseBooleanField(
    formData.getAll(COLUMN_MAP.gatherings),
    DEFAULT_PREFERENCES.gatherings
  ),
...
export const actions: Actions = {
  ...
  deleteAccount: async ({ locals }) => {
    ...
    if (
      ...
      console.error(
        '[mypage] Failed to delete endorsements during account cleanup',
        deleteEndorsements.error
      );
      return fail(500, {
    ...
    if (
      ...
      console.error(
        '[mypage] Failed to delete invites during account cleanup',
        deleteInvites.error
      );
      return fail(500, {
    ...
    if (deleteProfile.error && !isPermissionError(deleteProfile.error)) {
      console.error(
        '[mypage] Failed to delete profile during account cleanup',
        deleteProfile.error
      );
      return fail(500, {
