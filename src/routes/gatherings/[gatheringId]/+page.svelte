...
<main class="mx-auto flex w-full max-w-5xl flex-col gap-8 px-5 pb-16 pt-14 sm:px-8">
  <a
    class="inline-flex w-fit items-center gap-1 text-sm font-semibold text-blue-600 transition hover:text-blue-700"
    href="/gatherings"
  >
    ← 모임 목록으로
  </a>
  ...
  <article class="glass-panel space-y-6">
    <header class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
      <div class="flex items-center gap-4">
        ...
        />
        <div>
          <p class="text-base font-semibold text-peer-navy">
    ...
    {#if editingPost}
      <form class="space-y-4" method="post" action="?/updatePost" onsubmit={handlePostUpdateSubmit}>
    ...
    {:else}
      <ul class="space-y-4">
        {#each comments as comment}
          <li
            class="rounded-3xl border border-slate-200/60 bg-white/85 p-5 shadow-sm backdrop-blur"
          >
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div class="flex items-start gap-3">
                ...
                />
                <div>
                  <p class="text-sm font-semibold text-peer-navy">
            ...
            {:else}
              <p
                class="mt-3 whitespace-pre-wrap break-words text-sm leading-relaxed text-slate-700"
              >
                {comment.content}
              </p>
            {/if}
                  ...
                  <li class="rounded-2xl border border-slate-200/60 bg-slate-50/70 p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div class="flex items-start gap-3">
                        ...
                        />
                        <div>
                          <p class="text-xs font-semibold text-peer-navy">
