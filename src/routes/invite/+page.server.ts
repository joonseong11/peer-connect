...
const fetchInviteCards = async (
  ...
  for (const rawInvite of data ?? []) {
    ...
    const redemptions: InviteRedemptionRecord[] = redemptionsRaw.map((entry) => {
      ...
      return {
        ...
        invitee: inviteeRaw
          ? {
              full_name: typeof inviteeRaw.full_name === 'string' ? inviteeRaw.full_name : null,
  ...
  const cards: InviteCardState[] = slots.map((slot) => {
    ...
    const remainingUses =
      slot.maxRedemptions === null ? null : Math.max(slot.maxRedemptions - redemptionCount, 0);
...
const redeemInviteCode = async ({
  ...
  if (INVITE_FALLBACK_CODE && normalizedCode === INVITE_FALLBACK_CODE) {
    const { count: totalRedemptions } = await locals.supabase
      .from('invite_redemptions')
    ...
    const { data: existingFallbackInvite, error: fallbackLookupError } = await locals.supabase
      .from('invites')
    ...
    if (!fallbackInviteId) {
      const { data: insertedFallback, error: insertFallbackError } = await locals.supabase
        .from('invites')
        .insert({
          code: normalizedCode,
    ...
    const { count: fallbackUsageCount } = await locals.supabase
      .from('invite_redemptions')
    ...
    const { error: fallbackRedeemError } = await locals.supabase.from('invite_redemptions').insert({
...
const requiresLinkedInviteMessage = '먼저 초대 코드를 연결해야 초대장을 사용할 수 있어요.';
...
export const load: PageServerLoad = async ({ locals, url, cookies }) => {
  ...
  const { data: redeemedInvite, error: redeemedError } = await locals.supabase
    .from('invite_redemptions')
    .select(
  ...
  if (redeemedInvite) {
    ...
    if (rawInvite) {
      const rawInviter = Array.isArray(rawInvite.inviter)
        ? rawInvite.inviter[0]
        : rawInvite.inviter;
      redeemedSummary = {
        ...
        invite: {
          ...
          inviter: rawInviter
            ? {
                full_name: typeof rawInviter.full_name === 'string' ? rawInviter.full_name : null,
