  ...
  <section class="glass-panel space-y-4">
    ...
    {#if invitesEnabled}
      <p class="text-slate-600">
        신뢰할 수 있는 동료를 초대해 네트워크를 확장하세요. 초대링크로 가입한 멤버는 가입 직후
        당신의 프로필에서 추천을 남기도록 안내받습니다.
      </p>
    {:else}
      <p class="text-slate-600">
        현재는 초대 기능을 테스트 목적으로 비활성화해 두었습니다. 로그인 후 바로 프로필을 작성하고
        동료들을 둘러볼 수 있습니다.
      </p>
    {/if}
  ...
  {:else if invitesEnabled}
    <section class="glass-panel space-y-4">
      ...
      <p class="text-sm text-slate-500">
        초대받은 코드 1개를 입력하면 커뮤니티 이용이 활성화됩니다.
      </p>
      <form method="post" action="?/redeem" class="space-y-3" onsubmit={handleRedeemSubmit}>
            ...
            >
              ...
              {#if card.invite}
                ...
                {#if isExpanded(card.slot.index)}
                  <section
                    class="space-y-4 rounded-2xl border border-slate-200/60 bg-white/90 p-4 text-sm text-slate-600"
                  >
                    <div class="space-y-2">
                      ...
                      <p
                        class="rounded-2xl bg-peer-navy/90 px-4 py-2 text-lg font-semibold text-white"
                      >
                        {card.invite.code}
                      </p>
                    </div>
                    {#if shareUrl}
                      <div
                        class={`space-y-2 ${card.state === 'fulfilled' ? 'text-slate-500' : ''}`}
                      >
                        <h4 class="text-xs font-semibold uppercase tracking-wide text-slate-400">
                          공유 링크
                        </h4>
                    ...
                    <div class="space-y-2">
                      ...
                      {#if card.state === 'unlimited'}
                        <p
                          class="rounded-2xl bg-peer-indigo/10 px-3 py-2 text-xs font-semibold text-peer-indigo"
                        >
              ...
              {:else}
                <section
                  class="space-y-3 rounded-2xl border border-dashed border-peer-indigo/30 bg-white/80 p-4"
                >
                  <p class="text-xs text-slate-500">
