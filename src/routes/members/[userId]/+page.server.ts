...
export const actions: Actions = {
	endorse: async ({ request, locals, params }) => {
		...
		if (existing) {
			return fail(400, {
				success: false,
				errors: { content: '이미 이 동료에게 작성한 추천이 있습니다. 삭제 후 다시 작성할 수 있어요.' },
				values: { content }
			});
		}
		...
		if (emailColumnAvailable) {
			if (targetProfile?.email) {
				await notifyEndorsementReceived({
					...
					authorName: authorProfile?.full_name ?? session.user.user_metadata.full_name ?? '알 수 없는 동료',
					content
				});
			} else {
				console.warn(
